...
function ProposalCard({title, description}) {
    return (
        <div className="flex flex-col gap-2 p-6 bg-gray-50 rounded">
...
export default function Problem() {
    const proposalData = [
        {title : "GreenTech Innovation Grant", description : "A proposal focused on developing biodegradable batteries that provide long-lasting energy storage without harming the environment. The goal is to replace lithium-ion batteries with sustainable alternatives."},
        {title : "Next-Gen EdTech Platform", description : "A gamified learning app that teaches kids financial literacy through interactive storytelling and virtual investments. The app aims to make learning about money fun and engaging for children aged 8–14."},
        {title : "AI-Powered Remote Health Diagnostics", description : "A telemedicine service with smart wearables that track vital signs and detect early symptoms of chronic diseases like diabetes and heart conditions, helping doctors provide remote consultations."}
    ]

    const evaluatorData = [
        {name : "Dr. Evelyn Carter", title : "Renewable Energy Scientist & Rock Climber", src : "https://images.unsplash.com/photo-1505840717430-882ce147ef2d?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"},
        {name : "David Chang", title : "Surgeon & Street Food Critic"},
        {name : "Sophia Lin", title : "Venture Capitalist & Amateur Beekeeper", src : "https://images.unsplash.com/photo-1501031170107-cfd33f0cbdcc?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"}
    ]
    ...
    useEffect(() => {
        let size = getCurrentWindowSize()

        let isMobile = size.width < 768

        if (isMobile) {
            setProposalDatas(proposalData.slice(0, 1))
            setEvaluatorDatas(evaluatorData.slice(0, 1))
        }

    }, []);

    return (
        <article className="flex flex-col gap-6 sm:gap-12 justify-start">
            ...
            <div className="grid md:grid-cols-3 gap-6">
                {proposalDatas.map((proposal, index) => (
                    <ProposalCard key={index} title={proposal.title} description={proposal.description}/>
                ))}
             </div>
            <div className="flex gap-12">
                <svg xmlns="http://www.w3.org/2000/svg" className="hidden md:block md:w-full" width="477" height="194"
                 ...
                 <video src={video} alt="" loop autoPlay muted controls={false}/>
                {/*<img src={image} alt="" className="size-48"/>*/}
            </div>
            <div className="grid md:grid-cols-3 gap-6">
                {evaluatorDatas.map((evaluator, index) => (
